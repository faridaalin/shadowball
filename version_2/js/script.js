...
btnCart.addEventListener("click", () => {
  if (
    ...
    Number(size.options[size.selectedIndex].value) > 0
  ) {
    modal.style.display = "block";
    cartIcon.style.display = "block";
    document.querySelector(".itemNumber span").textContent = Number(
      qtyInput.value
    );
    validateFieldSize();
    validateFieldQty();
...
let date = `${year} &copy; Shadowball. All rights reserved.`;

const footer = (document.querySelector("#copywrightYear").innerHTML = date);
